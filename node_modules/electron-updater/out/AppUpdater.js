...
class AppUpdater extends _events().EventEmitter {
  ...
  getOrCreateStagingUserId() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const file = path.join(_this7.app.getPath("userData"), ".updaterId");
  ...
  executeDownload(taskOptions) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      if (_this8.listenerCount(_main().DOWNLOAD_PROGRESS) > 0) {
        downloadOptions.onProgress = it => _this8.emit(_main().DOWNLOAD_PROGRESS, it);
      }
      ...
      const cacheDir = _this8.downloadedUpdateHelper.cacheDir;
      ...
      function () {
        var _ref = (0, _bluebirdLst().coroutine)(function* (isSaveCache) {
          _this8.downloadedUpdateHelper.setDownloadedFile(updateFile, packageFile, updateInfo, fileInfo);

          if (isSaveCache) {
            yield _this8.downloadedUpdateHelper.cacheUpdateInfo(updateFileName);
          }
      ...
      const log = _this8._logger;
      const cachedUpdateFile = yield _this8.downloadedUpdateHelper.validateDownloadedPath(updateFile, updateInfo, fileInfo, log);
      ...
      function () {
        var _ref2 = (0, _bluebirdLst().coroutine)(function* () {
          yield _this8.downloadedUpdateHelper.clear().catch(() => {// ignore
          });
      ...
      try {
        ...
        if (e instanceof _builderUtilRuntime().CancellationError) {
          ...
          _this8.emit("update-cancelled", updateInfo);
        }
